...
Title.TextSize = 18
Title.Text = "Geta Pro IY FE v"..ver
...
CMDs[#CMDs + 1] = {NAME = 'copycframe', DESC = 'Copies your HumanoidRootPart CFrame to your clipboard'}
...
CMDs[#CMDs + 1] = {NAME = 'espnochams', DESC = 'No-chams | View all players and their status'}
...
addcmd('copycframe',{}, function(args, speaker)
	local cpcframe = tostring(game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame)
	toClipboard(cpcframe)
end)
...
addcmd('espnochams',{},function(args, speaker)
	if not CHMSenabled then
		ESPenabled = true
		for i,v in pairs(Players:GetChildren()) do
			if v.ClassName == "Player" and v.Name ~= speaker.Name then
...
task.spawn(function()
	if pcall(function() loadstring(game:HttpGet('https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/version'))() end) then
		if ver ~= Version then
			notify('DOG WATER EGG','DOG DOG DOG DOG DOG DOG')
		end
